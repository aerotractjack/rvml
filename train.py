...
def get_config(runner, **kw) -> ObjectDetectionConfig:
    # Parse and process the config file for this experiment
    input_config = pre_pipeline(kw)
    # Build the class config for our experiment
    class_config = ClassConfig(
        names=input_config["class_config"]["names"],
        colors=input_config["class_config"]["colors"])
    # Construct the train and val datasets from our lists of images/aois/labels
    ...
    # Define the options for windows/chips in our dataset
    ...
    # Combine all the window/chip config and scenes into a dataset object
    ...
    # Define the model/backend we are going to use for training/prediction
    ...
    # Define confidence threshold and merge options for making predictions
    predict_options = ObjectDetectionPredictOptions(**input_config["predict_options"])
    # Combine all previos configs together into one big experiment bundle
    return ObjectDetectionConfig(
        root_uri=input_config["output_root_uri"],
