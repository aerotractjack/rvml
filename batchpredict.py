...
def batch_predict(idx):
    ...
    with open(f'tifpathlists/{idx}.txt','r') as text:
    ...
    for bundle_uri in bundle_list:
        for number,tif in enumerate(prediction_list):
            ...
            tif_d = generate_output_path(tif)
            # downsample_tif(tif, tif_d)
            print(output_directory)
            print(tif, tif_d)
            print(bundle_uri)

            # subprocess.run(['rastervision','predict',bundle_uri,tif_d,output_directory])
